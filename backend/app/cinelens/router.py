"""
CineLens recommender feature, integrated into the main movie-website API.

This is the original CineLens FastAPI app (recommender bake-off: item-based
CF vs content-based vs hybrid) ported to an APIRouter so it can be mounted
under the main app instead of running as a second server. Endpoints are
unchanged (same paths, same request/response shapes) other than living
under the "/api/cinelens" prefix, and are now gated behind the site's
existing login (no separate CineLens login).
"""
import random
import time
...
router = APIRouter(
    prefix="/api/cinelens",
    tags=["cinelens"],
    dependencies=[Depends(get_current_user)],  # reuse the site's existing login
)
...
@router.get("/movies/similar", response_model=SimilarMoviesResponse)
def movies_similar(
    ...
    `title` must match exactly (including year); use /api/cinelens/movies/search to
    ...
    if results is None:
        raise HTTPException(
            status_code=404,
            detail=f'"{title}" not found in the ratings dataset. Use /api/cinelens/movies/search to find the exact title.',
        )
